    ...
    const checkQuizStatus = async () => {
        ...
        try {
            const res = await fetch(
                `${import.meta.env.VITE_API_URL}/api/quiz/attempt/status?userId=${userId}&articleId=${id}`,
            ...
            console.error("Gagal cek status kuis:", err);
        }
    };
            ...
            <div className="min-h-screen px-6 md:px-10 py-16 max-w-3xl mx-auto w-full">
                ...
                <motion.article {...fadeUp} className="flex flex-col gap-6">
                    ...
                    <button
                        ...
                        disabled={hasPassed}
                        title={hasPassed ? "Kamu sudah lulus kuis ini" : undefined}
                        className={`w-full mt-6 py-3 rounded-xl font-medium transition
                        ${hasPassed
                            ? "bg-green-800/40 text-green-400/60 cursor-not-allowed"
                            : "bg-blue-600 hover:bg-blue-700 text-white"
                        }`}
                    >
                        {hasPassed ? "Quiz Sudah Diselesaikan" : "Mulai Quiz"}
                    </button>
                </motion.article>
            </div>
        </>
