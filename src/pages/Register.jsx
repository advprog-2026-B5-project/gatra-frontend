...
const Register = () => {
    ...
    const [formData, setFormData] = useState({ username: '', email: '', displayName: '', password: '', repeatPassword: '' });
    ...
    return (
        <div className="flex-1 w-full relative overflow-hidden flex flex-col">
            ...
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-200 h-125 bg-purple-600/20 blur-[120px] rounded-t-full pointer-events-none"></div>

            {/* Main Content */}
            <div className="flex-1 flex items-center justify-center p-4 relative z-10 pb-10 pt-10">
                <div className="bg-[#131627] p-8 md:p-10 rounded-3xl w-full max-w-100 shadow-2xl border border-gray-800/60">
                    ...
                    <form onSubmit={handleRegister} className="space-y-4">
                        <div>
                            <label className="block text-xs text-gray-400 mb-1.5 font-medium ml-1">Username</label>
                            <input type="text" required placeholder="Type here" value={formData.username}
                        ...
                        <div>
                            ...
                            <input type="text" required placeholder="Type here" value={formData.email}
                                   onChange={(e) => setFormData({...formData, email: e.target.value})}
                                   className="w-full bg-[#1E2235] border border-transparent focus:border-blue-500 rounded-xl p-3 text-sm outline-none text-white placeholder-gray-500 transition" />
                        </div>
                        <div>
                            <label className="block text-xs text-gray-400 mb-1.5 font-medium ml-1">Display name</label>
                            <input type="text" required placeholder="Type here" value={formData.displayName}
                                   onChange={(e) => setFormData({...formData, displayName: e.target.value})}
                                   className="w-full bg-[#1E2235] border border-transparent focus:border-blue-500 rounded-xl p-3 text-sm outline-none text-white placeholder-gray-500 transition" />
                        </div>
                        <div>
                            <label className="block text-xs text-gray-400 mb-1.5 font-medium ml-1">Password</label>
                            <input type="password" required placeholder="Type here" value={formData.password}
                                   onChange={(e) => setFormData({...formData, password: e.target.value})}
                                   className="w-full bg-[#1E2235] border border-transparent focus:border-blue-500 rounded-xl p-3 text-sm outline-none text-white placeholder-gray-500 transition" />
                        </div>
                        <div>
                            <label className="block text-xs text-gray-400 mb-1.5 font-medium ml-1">Repeat password</label>
                            <input type="password" required placeholder="Type here" value={formData.repeatPassword}
